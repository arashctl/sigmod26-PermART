...
/* Pub-Elim-ABtree code starts here */
template <typename K>
struct kvpair {
    K key;
    void * val;
  ...
  public:
    ...
    class LockGuard {
      ...
      public:
        ...
        */
        tuple<bool, V> acquire(const int tid, const K key = (K) 0, const bool shortcircuit = false, const uint32_t oldestWV = 0) {
            ...
            // const uint32_t oldestWV = node->writeVersion.load(memory_order_acquire);
            while (true) {
                while (shortcircuit) {
                    uint32_t writeVer1;
                    ...
                    uint32_t elimVer = node->elimVer;
                    K elimKey = node->elimKey;
                    V elimVal = node->elimVal;
                    SOFTWARE_BARRIER;
                    uint32_t writeVer2 = node->writeVersion.load(memory_order_acquire);
        ...
        inline void elim(const K key, const V val, const uint32_t writeVersion) {
            assert(owned);
            assert(key != 0);
            assert(val != 0);
...
template <class RecordManager, typename K, typename V, int DEGREE, class Compare>
ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::~ABTreeOptik() noexcept(false) {
    int tid = 0;
    initThread(tid);
    freeSubtree(0, entry);
    deinitThread(tid);
...
int ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::insert(const int tid, SearchInfo &info, const K &key, const V &value) {
    ...
    if (currSize < b) {
        ...
        for (int i = 0; i < DEGREE; ++i) {
            if (node->keys[i] == (K) 0) {
                const uint32_t oldVer = node->writeVersion.load(memory_order_relaxed);
                node->writeVersion.store(oldVer + 1, memory_order_relaxed);
                SOFTWARE_BARRIER;
                leafLock.elim(key, value, oldVer + 1);
...
int ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::erase(const int tid, SearchInfo &info, const K &key) {
    ...
    for (int i = 0; i < DEGREE; ++i) {
        if (node->keys[i] == key) {
            ...
            node->writeVersion.store(oldVer + 1, memory_order_relaxed);
            SOFTWARE_BARRIER;
            leafLock.elim(key, info.val, oldVer + 1);
            node->keys[i] = (K) 0;
            node->size = newSize;
            flush(&node->keys[i]);
            node->writeVersion.store(oldVer + 2, memory_order_release);
...
void ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::fixUnderfullViolation(const int tid, SearchInfo& info) {
    ...
    NodeLockGuard nodeLock = NodeLockGuard(node);
    NodeLockGuard siblingLock = NodeLockGuard(sibling);
    int leftIndex;
    int rightIndex;
    nodeptr left = NULL;
    nodeptr right = NULL;
...
template <class RecordManager, typename K, typename V, int DEGREE, class Compare>
void ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::recoverNode(nodeptr node) {
    // Forcibly unlock nodes and fix any broken size fields or unflushed pointers
    assert(!node->marked);
    if (node->locked) {
        node->locked = false;
    }
    if (!node->leaf) {
        for (int i = 0; i < node->size; ++i) {
            if (dirty(node->ptrs[i])) {
                flushPtr(&node->ptrs[i]);
                recoverNode(node->ptrs[i]);
            }
        }
    }
    else {
...
template <class RecordManager, typename K, typename V, int DEGREE, class Compare>
void ABTreeOptik<RecordManager, K, V, DEGREE, Compare>::recover() {
    recoverNode(entry);
    sync();
    assert(validate());
}
