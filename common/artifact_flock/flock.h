...
namespace flck {
    ...
    auto try_loop(const F& f, int delay = 200, const int max_multiplier = 10) {
        ...
        while (true) {
            ...
            auto start = flck_rdtscp();
            #endif

            multiplier = std::min(2 * multiplier, max_multiplier);
            for (volatile int i = 0; i < delay * multiplier; i++);
            #ifdef MEASURE_COW_TIME
            auto end = flck_rdtscp();
            if (flck::thread_is_inserting && flck::measuring_cow_time) {
                GSTATS_ADD(flck::setbench_tid, insert_time_lock_wait, end - start);
            }
            #endif
        }
        #endif 
