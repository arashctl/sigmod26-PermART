    ...
    namespace internal {
            ...
            void update_epoch(const int tid) {
                ...
                if (all_there) {
                    ...
                    if (current_epoch.compare_exchange_strong(current_e, current_e + 1)) {
                        GSTATS_ADD(0, epoch_increments, 1);
        ...
        private:
            ...
            void clear_list(const int tid, Link* ptr) {
                ...
                long start_clear_list = my_rdtscp();

                if (ptr != nullptr) {
                    GSTATS_ADD(tid, clear_list_calls, 1);
                }
                ...
                long end_clear_list = my_rdtscp();
                long duration = end_clear_list - start_clear_list;
                GSTATS_ADD(tid, clear_list_time, duration);
            }
            ...
            nodeT* allocate_node(const int tid) { 
                GSTATS_ADD(tid, ralloc_alloc_calls, 1);
                return (nodeT*)malloc(sizeof(nodeT)); 
            }
            void free_node(const int tid, nodeT* x) { 
                GSTATS_ADD(tid, ralloc_free_calls, 1);
                return free(x); 
            }
#elif USE_RALLOC
            ...
            nodeT* allocate_node(const int tid) { 
                GSTATS_ADD(tid, ralloc_alloc_calls, 1);
                return (nodeT*)lvmlc_malloc(sizeof(nodeT)); 
            }
            void free_node(const int tid, nodeT* x) { 
                GSTATS_ADD(tid, ralloc_free_calls, 1);
                return lvmlc_free(x); 
